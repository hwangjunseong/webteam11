...
@app.route('/add_crollpost', methods=['POST', 'GET'])
@login_required
def add_crollpost():
    ...
    if form.validate_on_submit():
        ...
        if OldCategory is None:
            ...
            return redirect(url_for('add_crollpost'))
    ...
    #addcroll post에 categoriest 다전달해야함
    categories = Categories.query.order_by(Categories.date_posted.desc())
    return render_template('add_crollpost.html',categories = categories,form = form, current_user = current_user)
...
def start_crolling(category_url, category_name):
...
@app.route('/crolling' , methods=['GET', 'POST'])
def crolling():
    ...
    if request.method == 'POST':
        ...
        if category_name == "Choose...":
            ...
            start_crolling(category_url, category_name)
    return render_template('crolling.html')
